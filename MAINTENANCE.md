...
| **Table de membres de campagne** (`campaign_members`) | Actuellement, rejoindre une room ne crée aucun lien en base entre l'utilisateur et la campagne (limite documentée dans les policies RLS) ; nécessaire pour afficher la vraie liste des joueurs (`RoomPlayersPage`, aujourd'hui un placeholder) | Fort (débloque une fonctionnalité et corrige un angle mort de sécurité/traçabilité) | Moyen (nouvelle table + policies RLS + UI) |
